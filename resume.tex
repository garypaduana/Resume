...
\cventry{Present \\ May 2015}{Software Engineer}{\href{http://www.manifoldtechnology.com}{{\color{LinkBlue}Manifold Technology}}}{Sunnyvale, CA}{}{Helped architect, develop, deploy, and maintain private blockchain software in a startup environment.
	\begin{itemize}
		\item Responsible for all aspects of full-stack development due to the small team size.
		\item Contributed to the core backend product, which is an asynchronous and highly concurrent collection of Spring Boot Java applications with REST APIs and PostgreSQL and Cassandra data stores.
		\item Created a test client to measure the performance of the core platform under various load scenarios.
		\item Managed cloud architecture assets in Amazon Web Services and Microsoft Azure.
		\item Performed Linux administration to include creating Docker images, writing RPM specifications, building local test servers with ZFS data pools, managing RSA private keys and SSL certificates, SSH tunnels and forwarding, and rsync for deployments and artifact collection, among many other tasks.
		\item Configured and maintained a continuous integration environment with Jenkins that automatically deployed the latest builds to AWS EC2 instances.
...
\cventry{Since \\ Jan 2015}{\href{http://www.garypaduana.com}{{\color{LinkBlue}Personal Website}}}{}{}{}{A collection of personal projects with live functionality, demonstrations, documentation, and user guides.  Implemented with Grails and Bootstrap; hosted with DigitalOcean.
	\begin{itemize}
		\item \textit{Portrait Photos} - Currently hosting over 80 professional headshot image galleries for clients that I've photographed.  Each gallery is built using a custom automated workflow that depends on \textit{fgallery} and made private with a personalized URL.
		\item \textit{Capital Gains Calculator} - Analyzes transaction history using FIFO accounting and keeps track of short-term and long-term gains, wash sales, proceeds, and cost basis spanning multiple years of transaction history.  A demonstration feature generates random transaction history based upon historical daily Bitcoin pricing values to show a variety of tax scenarios.  Output is neatly organized using Bootstrap tables to allow one to follow each transaction in fine detail.
		\item \textit{Word Solver} - A browser implementation of my later mentioned Android application. 
		\item \textit{k-Means Clustering} - Provides a video demonstration of the algorithm in action running on an Android device, pseudocode for the view and controller, and UML class diagrams for the model.  Source code and a runnable .apk file are available for download.
		\item \textit{Blog} - Striving for useful and succinct posts regarding technology, programming, engineering, and problem solving.
		\item \textit{Photography} - Contains a collection of photos I've taken presented using Bootstrap Image Gallery.
	\end{itemize}
}
